...
from utils.metrics import acc_metric,f1_score
import torch
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.scale:
            #불러오기 
            print('df_data',df_data.iloc[:,:-1].shape)
            scaler = load(open(self.scaler_path, 'rb'))
            data = scaler.transform(df_data.iloc[:,:-1].values)
...
def predict(args):
    ...
    preds_th = np.where(predict_values>=0.5,1,0) #classification
    pd.DataFrame(preds_th, columns=[cols+[target]]).to_excel(f'{target}_prediction.xlsx')
    
    
    # 예측 성능 
    for i in range(len(preds_th)):
        df_raw.loc[args.seq_len+i,'pred']  = int(preds_th[i][-1])
    
    df_eval =df_raw[df_raw['pred'].notnull()]
    acc = acc_metric(df_eval[target],df_eval['pred'])
    f1,recall = f1_score(df_eval[target],df_eval['pred'])
    ...
    # 모델기록
    # 1. 범주형predict 코드 작성해야함
    # 2. 각 시기별 mae,mse,f1,recall,acc 구해야함
...
if __name__=='__main__':
    ...
    best_model_path = '/home/seoul/홍기영/스마트팜test/checkpoints/flow_fan/test_Informer_custom_ftMS_sl48_ll48_pl1__dm512_nh8_el2_dl1_df2048_fc1_ebfixed_dtTrue_test_0/acc_0.921484_f1_0.928223_recall_0.91698_loss0.0601511_checkpoint.pth'
