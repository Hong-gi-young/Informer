...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            # data + target 값 
            targets_data = df_raw[[self.target]].values
            data = np.concatenate((data,targets_data), axis=1)
            print('data+concat shape',data.shape)
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, vali_acc, vali_f1, vali_recall, model, path):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, vali_acc, vali_f1, vali_recall, model, path)
    ...
    def save_checkpoint(self, val_loss, vali_acc, vali_f1, vali_recall, model, path):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {vali_f1:.6f}).  Saving model ...')
        torch.save(model.state_dict(), path + '/' + f'acc_{vali_acc:.6}_f1_{vali_f1:.6}_recall_{vali_recall:.6}_loss{val_loss:.6}_checkpoint.pth')
        self.val_loss_min = vali_f1        
...
class Exp_Main():
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        preds_acc = np.where(preds>=0.5,1,0)
        ...
        return total_loss,acc,f1,recall
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            vali_loss, vali_acc,vali_f1,vali_recall = self.vali(vali_data, vali_loader, criterion)
            ...
            early_stopping(vali_loss,vali_acc, vali_f1, vali_recall, self.model, path)
            print('\n')
    ...
    def test(self, setting, test=0):
        ...
        f = open("result.txt", 'a')
        f.write(setting + "  \n")
        f.write('test_f1: {}, test_recall: {:.5f}, test_acc: {}'.format(f1, recall, acc))
        f.write('\n')
        f.write('\n')
        f.close()

        np.save(folder_path + 'test_acc_pred.npy', preds_acc)
...
if __name__ == '__main__':
    ...
    targets_cls = ['flow_fan','hmdfc','crc_pump','heat_cooler']
    target = 'heat_cooler'
